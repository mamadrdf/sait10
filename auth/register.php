<form class="form-auth" action='/register-auth.php' method="post">
    <img class="mb-4" src="../assets/img/enter.svg" alt="" width="72" height="72">
    <h1 class="h3 mb-3 font-weight-normal">عضویت در وبسایت</h1>
    <input type="email" id="inputEmail" class="form-control"  name="name" placeholder="نام و نام خانوادگی" required autofocus>
    <input type="email" id="inputEmail" class="form-control"  name="email" placeholder="آدرس ایمیل" required autofocus>
    <input type="password" id="inputPassword" class="form-control" name="password" placeholder="رمز عبور" required>
    <div class="checkbox mb-3">
  
    </div>
    <button class="btn btn-lg btn-primary btn-block" type="submit">ورود</button>
  </form>